...
class Link():
    ...
    def _genkey(self):
        proc = subprocess.Popen(
            '%s --genkey --secret /dev/stdout' % (self.OpenVPNPath),
    ...
    def __init__(self, serverrouter, clientrouter, linkport, iface_number, block):
        # BUG: Hack because Ubuntu 11.10 moved openvpn from /bin to /sbin and the user doesn't have a path to /sbin.
        # Need to search path, then check /sbin
    ...
    def __unicode__(self):
        return 'Server: %s, Client: %s, Port: %s, Iface: %s Block: %s' % (
            self.server.fqdn,
            self.client.fqdn,
            self.port,
            self.iface,
