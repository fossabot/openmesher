...
class Shorewall(IOpenMesherConfigPlugin):
    ...
    def process(self, mesh, cliargs=None):
        ...
        for router in mesh.links:
            self._files[router] = {}
            self._files[router]['/shorewall/interfaces.mesh'] = self._templates['shorewall/interfaces.conf'].render(
                links=mesh.links[router],
                vpnzone=cliargs.vpnzone,
                fwzone=cliargs.fwzone,
                wanzone=cliargs.wanzone
            )
            self._files[router]['/shorewall/rules.mesh'] = self._templates['shorewall/rules.conf'].render(
                links=mesh.links[router],
                vpnzone=cliargs.vpnzone,
                fwzone=cliargs.fwzone,
                wanzone=cliargs.wanzone
