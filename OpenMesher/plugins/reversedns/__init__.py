        ...
        for router in mesh.links:
            for link in mesh.links[router]:
                if link.isServer(router):
                    ...
                    rdns.write(self._templates['reversedns/reverse.conf'].render(
                        ip1=ip1,
                        ip2=ip2,
                        link=link,
                        links=mesh.links[router]
                    ))
                    dnsmasq.write(self._templates['reversedns/dnsmasq.conf'].render(
                        ip1=ip1,
                        ip2=ip2,
                        link=link,
                        links=mesh.links[router]
                ...
                self._files[router] = {
                    '/mesh-reverse.db': rdns.getvalue(),
                    '/dnsmasq.d/mesh-reverse.conf': dnsmasq.getvalue()
                }
